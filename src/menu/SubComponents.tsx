...
export const MenuSection = ({ children, style }: { children: React.ReactNode; style?: string; }) => {
  return <section className={`min-w-full h-full overflow-scroll scrollbar-none`}>
    <div className={`w-full h-full pt-[80px] px-[30px] flex flex-col ${style ?? ""}`}>
      {children}
    </div>
  </section>;
};
...
export const Title = ({ text, style }: { text: string; style?: string; }) => {
  return <h2 className={`mb-[20px] font-bold text-center md:text-lg ${style ?? ""}`}>{text}</h2>;
};
...
export const ConfirmedInput = ({
  ...
  return (
    <div className={`flex gap-[20px] flex-col items-center ${style ?? ""}`}>
      <input
        type="text"
        placeholder={placeholder}
        value={input}
        className={`w-full h-full max-w-[600px] bg-transparent border-b border-[var(--text-color)] rounded-none outline-none text-center py-[5px] placeholder:text-[var(--text-color)] placeholder:opacity-50`}
