...
export default function RoomPage() {
    ...
    //         // navigate('/');
    //         setRoomId(GLOBAL_ROOM);
    //     } else {
    //         console.log('heh: ', path);
    //         setRoomId(path);
    //     }
    // }, [location]);
    ...
    return (
        <div className="w-full h-full grid place-items-center bg-[var(--primary)] relative">
            ...
            {/* Action dialog */}
            <button className={`absolute top-0 w-[100px] h-[30px] bg-white shadow-md rounded-[24px] transition-all duration-500 ${dialogVisible ? "-translate-y-full" : "-translate-y-1/3"}`}
                ...
                <>
                    ...
                    <div className="flex flex-col justify-center gap-[25px] p-[30px]">
                        ...
                        <div className="flex justify-center">
                            ...
                            {action === "JOIN" && (
                                <button className={`font-bold text-white px-[30px] py-[5px] rounded-[24px] ${roomId ? 'bg-[var(--secondary)]' : 'bg-gray-500'}`}
                                    onClick={() => joinRoom()}>
                                    Join
                                </button>
                            )}
